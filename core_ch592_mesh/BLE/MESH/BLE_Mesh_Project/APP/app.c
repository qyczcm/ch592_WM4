...
uint8_t self_prov_net_key[16] = {0};  // ������Կ����static���ɱ��ⲿ���ʣ�

const uint8_t prov_tube_dev_key[16] = {//��������Կ
    0x3F, 0x92, 0x5A, 0xA3, 0x6B, 0x2C, 0x8D, 0x4F, 0x01, 0x76, 0xB9, 0x34, 0xC7, 0xF0, 0x85,0x86
};
...
static void prov_complete(uint16_t net_idx, uint16_t addr, uint8_t flags, uint32_t iv_index)
{
    ...
    my_net_addr = addr;
    APP_DBG("sssssss%dssss",my_net_addr);
    Mesh_is_Provisioned_Flag = true;
...
static void vendor_model_srv_rsp_handler(const vendor_model_srv_status_t *val)
{
    ...
    if(val->vendor_model_srv_Hdr.opcode == OP_VENDOR_MESSAGE_TRANSPARENT_MSG)
    {
        ...
        APP_DBG("len %d, data 0x%02x from 0x%04x", val->vendor_model_srv_Event.trans.len,
                ...
                if(my_net_addr != val->vendor_model_srv_Event.trans.addr)//mesh�յ������� �������
//                UART0_SendString(val->vendor_model_srv_Event.trans.pdata, val->vendor_model_srv_Event.trans.len);
...
void blemesh_on_sync(void)
{
    ...
    // �͹���ģʽ && �ǵ���ģʽ����������رչ㲥
    #if(CONFIG_BLE_MESH_GBO_POWER)
        if(!Debug_Always_ADV_Flag)
        {
            uint8_t initial_advertising_enable = FALSE;
            GAPRole_SetParameter(GAPROLE_ADVERT_ENABLED, sizeof(uint8_t), &initial_advertising_enable);
            APP_DBG("Advertising disabled (provisioned + power saving mode)");
        }
        else
        {
            APP_DBG("Advertising enabled (DEBUG mode active)");
        }
    #endif
...
void App_Init()
{
    ...
    blemesh_on_sync();
    Peripheral_Init();//�ŵ�����������ΪҪ�Ȼ��my_net_addr��Ȼ�����������
//    HAL_KeyInit();
...
void App_peripheral_reveived(uint8_t *pValue, uint16_t len)
{
    ...
    switch(app_mesh_manage.data.buf[0])
    {
        ...
        // �������� ���� 1�ֽ�������+16�ֽ�������Կ+2�ֽ������ַ ��ʾ��A1+8c3e9f7a2d1b6c4a5e8f3a2b7c6d5e4f+0001
        case CMD_PROVISION:
        {
...
void Mesh_Send_Data(uint16_t addr,uint8_t *pValue, uint16_t len)
{
    uint8_t status;
    status = vendor_model_srv_send(addr, pValue, len);//BLE_MESH_ADDR_ALL_NODES - 0xffff
    if(status)
    {
        APP_DBG("send failed %d", status);
    }
